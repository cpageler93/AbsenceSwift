...
public class AbsencesFilter {
    ...
    public enum DateItem {
        ...
        func bodyItem() -> (String, String) {
            switch self {
            case .lte(let date): return ("$lte", AbsenceDateFormatter.string(from: date))
            case .gte(let date): return ("$gte", AbsenceDateFormatter.string(from: date))
            }
        }
    }
